...
def test_ci_include(tmpdir, config):
    test_yaml = {
        "ci": {
            "list": [1,2,3],
            "script": ["a","b","c"],
    ...
    ci.ci_process_includes(test_yaml, include=inc_yaml)
    ...
    def check(result, expected):
        for key, val in expected.items():
            if isinstance(val, dict):
                ...
                assert all([x == y for x, y in zip(val, result[key])])
            else:
                assert val == result[key]


    check(test_yaml, expected_yaml)
