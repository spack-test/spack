...
def generate_gitlab_ci_yaml(
    ...
    if copy_only_pipeline and not config_deprecated:
        ...
        if "buildcache-source" in pipeline_mirrors:
            ...
            stack_name = os.environ.get("SPACK_CI_STACK_NAME", None)
            last_bit = f"/{stack_name}" if stack_name else ""
            buildcache_source = f"s3://spack-binaries/SPACK_REPLACE_VERSION{last_bit}"
